...
export class LobbyMenu extends React.Component<{ onLeave: () => void }, { elements: { [id: string]: type_button_info }, map: string, members: string[], isOwner: boolean, name: string }> {
    constructor(props) {
        super(props);
    ...
    updateLobby(state: { isLeader: boolean, map: string, name: string, players: string[] }) {
        this.setState({ map: state.map, name: state.name, members: state.players, isOwner: state.isLeader });
    }
    ...
    render() {
        ...
        if (this.state.isOwner) {
            title = <input className={styles["lobby-nameinput"]} type="text" value={this.state.name} onChange={this.submitName} placeholder="Lobby Name"></input>;
        } else {
            ...
            <div id="lobby-main" className={styles["lobby-main"]}>
                ...
                <div id="lobby-settings" className={styles["lobby-settings"]}>
                    <div className={styles["lobby-headerbar"]}>
                        <button className={styles["lobby-leavebutton"]} onClick={this.onLeaveClick}>Back</button>
                        {title}
                    </div>
                    <ButtonPanel canSelect={this.state.isOwner} onChange={this.handleMapChange} default={this.state.map} buttons={this.state.elements} title="Maps"></ButtonPanel>
                </div>
            </div>
